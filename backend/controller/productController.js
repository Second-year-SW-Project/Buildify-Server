...
export const createProduct = async (req, res) => {
  try {
        ...
        imagesUrl = await Promise.all(
          images.map(async (item) => {
            try {
              ...
              console.error(
                `Error uploading image: ${item.path}`,
                uploadError.message
              ); // Debugging
    ...
    res
      .status(201)
      .json({ Success: true, data: toCamelCase(newProduct.toObject()) });
  } catch (error) {
...
export const getProducts = async (req, res) => {
  try {
    ...
    if (
      req.query.limit &&
      (isNaN(parseInt(req.query.limit)) || parseInt(req.query.limit) <= 0)
    ) {
    ...
    if (
      ...
      !allowedStatus.includes(req.query.statusFilter)
    ) {
      return res
        .status(400)
...
export const updateProduct = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ Success: true, data: toCamelCase(updatedProduct.toObject()) });
  } catch (error) {
        ...
        await Promise.all(
          existingProduct.img_urls.map((img) =>
            cloudinary.uploader.destroy(img.public_id).catch((err) => {
              console.error(
                `Error deleting image: ${img.public_id}`,
                err.message
              );
            })
          )
...
export const getProductsByAttribute = async (req, res) => {
  try {
    ...
    if (manufacturer) {
      ...
      const manufacturersToFilter = Array.isArray(manufacturer)
        ? manufacturer
        : [manufacturer];
      const validManufacturers = manufacturersToFilter.filter(
    ...
    if (memoryType) {
      ...
      const memoryTypesToFilter = Array.isArray(memoryType)
        ? memoryType
        : [memoryType];
      const validMemoryTypes = memoryTypesToFilter.filter(
        (mt) => typeof mt === "string" && mt.trim() !== ""
      );
    ...
    if (coreCount) {
      ...
      const coreCountValues = Array.isArray(coreCount)
        ? coreCount
        : [coreCount];
    ...
    if (threadCount) {
      ...
      const threadCountValues = Array.isArray(threadCount)
        ? threadCount
        : [threadCount];
    ...
    if (socketType) {
      ...
      const socketTypesToFilter = Array.isArray(socketType)
        ? socketType
        : [socketType];
      const validSocketTypes = socketTypesToFilter.filter(
        (st) => typeof st === "string" && st.trim() !== ""
      );
    ...
    if (motherboardChipset) {
      ...
      const chipsetsToFilter = Array.isArray(motherboardChipset)
        ? motherboardChipset
        : [motherboardChipset];
      const validChipsets = chipsetsToFilter.filter(
    ...
    if (efficiencyRating) {
      ...
      const efficiencyRatingsToFilter = Array.isArray(efficiencyRating)
        ? efficiencyRating
        : [efficiencyRating];
      const validEfficiencyRatings = efficiencyRatingsToFilter.filter(
        (er) => typeof er === "string" && er.trim() !== ""
      );
    ...
    if (storageCapacity) {
      ...
      const storageCapacityValues = Array.isArray(storageCapacity)
        ? storageCapacity
        : [storageCapacity];
    ...
    if (storageType) {
      ...
      const storageTypesToFilter = Array.isArray(storageType)
        ? storageType
        : [storageType];
      const validStorageTypes = storageTypesToFilter.filter(
        (st) => typeof st === "string" && st.trim() !== ""
      );
    ...
    if (supportedMotherboardSizes) {
      ...
      const motherboardSizesToFilter = Array.isArray(supportedMotherboardSizes)
        ? supportedMotherboardSizes
        : [supportedMotherboardSizes];
      const validMotherboardSizes = motherboardSizesToFilter.filter(
    ...
    if (maxGpuLength) {
      ...
      const maxGpuLengthValues = Array.isArray(maxGpuLength)
        ? maxGpuLength
        : [maxGpuLength];
    ...
    if (ram) {
      ...
      const validRams = ramsToFilter.filter(
    ...
    if (graphicCard) {
      ...
      const graphicCardsToFilter = Array.isArray(graphicCard)
        ? graphicCard
        : [graphicCard];
      const validGraphicCards = graphicCardsToFilter.filter(
        (gc) => typeof gc === "string" && gc.trim() !== ""
      );
    ...
    if (storage) {
      ...
      const validStorages = storagesToFilter.filter(
        (s) => typeof s === "string" && s.trim() !== ""
      );
    ...
    if (componentType) {
      ...
      const componentTypesToFilter = Array.isArray(componentType)
        ? componentType
        : [componentType];
      const validComponentTypes = componentTypesToFilter.filter(
    ...
    if (displaySize) {
      ...
      const displaySizeValues = Array.isArray(displaySize)
        ? displaySize
        : [displaySize];
    ...
    if (panelType) {
      ...
      const panelTypesToFilter = Array.isArray(panelType)
        ? panelType
        : [panelType];
      const validPanelTypes = panelTypesToFilter.filter(
        (pt) => typeof pt === "string" && pt.trim() !== ""
      );
    ...
    if (refreshRate) {
      ...
      const refreshRateValues = Array.isArray(refreshRate)
        ? refreshRate
        : [refreshRate];
    ...
    if (interfaceType) {
      ...
      const interfaceValues = Array.isArray(interfaceType)
        ? interfaceType
        : [interfaceType];
      const validinterfaceValues = interfaceValues.filter(
        (m) => typeof m === "string" && m.trim() !== ""
      );
...
export const getProductById = async (req, res) => {
  try {
    ...
    if (!s_product) {
      return res
        .status(404)
        .json({ Success: false, message: `Product not found for ID: ${id}` });
    }
    ...
    res
      .status(200)
      .json({ Success: true, ...camelCasedProduct, _id: s_product._id });
  } catch (error) {
...
export const getProductCountsByMainCategory = async (req, res) => {
  try {
    ...
    if (!mainCategory || !subCategories[mainCategory]) {
      return res
        .status(400)
        .json({ Success: false, message: "Invalid main category" });
    }
...
export const getManufacturersByCategory = async (req, res) => {
  try {
    ...
    const products = await Product.find({ type: category }).distinct(
      "manufacturer"
    );
...
export const getPrebuildRamSizes = async (req, res) => {
  try {
    ...
    const ramSizes = await Product.find({ type: "prebuild" }).distinct(
      "ram_size"
    );
...
export const getLaptopGraphicCards = async (req, res) => {
  try {
    ...
    const graphicCards = await Product.find({ type: "laptop" }).distinct(
      "graphic_card"
    );
...
export const getMotherboardChipsets = async (req, res) => {
  try {
    ...
    const chipsets = await Product.find({ type: "motherboard" }).distinct(
      "motherboard_chipset"
    );
...
export const getPowerEfficiencyRatings = async (req, res) => {
  try {
    ...
    const efficiencyRatings = await Product.find({ type: "power" }).distinct(
      "efficiency_rating"
    );
...
export const getStorageCapacities = async (req, res) => {
  try {
    ...
    const storageCapacities = await Product.find({ type: "storage" }).distinct(
      "storage_capacity"
    );
...
export const getStorageTypes = async (req, res) => {
  try {
    ...
    const storageTypes = await Product.find({ type: "storage" }).distinct(
      "storage_type"
    );
...
export const getMaxGpuLengths = async (req, res) => {
  try {
    ...
    const maxGpuLengths = await Product.find({ type: "casing" }).distinct(
      "max_gpu_length"
    );
...
export const getMonitorDisplaySizes = async (req, res) => {
  try {
    ...
    const displaySizes = await Product.find({ type: "monitor" }).distinct(
      "display_size"
    );
...
export const getMonitorPanelTypes = async (req, res) => {
  try {
    ...
    const panelTypes = await Product.find({ type: "monitor" }).distinct(
      "panel_type"
    );
...
export const getMonitorRefreshRates = async (req, res) => {
  try {
    ...
    const refreshRates = await Product.find({ type: "monitor" }).distinct(
      "refresh_rate"
    );
...
export const getExpansionComponentTypes = async (req, res) => {
  try {
    ...
    const componentTypes = await Product.find({
      type: "expansion_network",
