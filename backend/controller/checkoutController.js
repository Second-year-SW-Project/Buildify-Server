...
export const deleteOrder = async (req, res) => {
  try {
    ...
    res.status(200).json({
      Success: true,
      message: `Order With #${existingOrder.id
        .slice(-4)
        .toUpperCase()} Id Deleted Successfully`,
    });
  } catch (error) {
...
export const getOrdersSummaryTotals = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch order summary", error: err.message });
  }
};
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    const updatedOrder = await Transaction.findByIdAndUpdate(id, updateData, {
      new: true,
    });
...
export const getBarChartSummary = async (req, res) => {
  try {
    ...
    xLabels.forEach((label) => {
      ...
      sales.push(
        group
          .filter((o) => o.status !== "Refunded" && o.status !== "Canceled")
      ...
      rawCancleOrders[label] = cancledOrders.map((o) => ({
        total: o.total,
        stepTimestamps: o.stepTimestamps,
      }));
    });
    res
      .status(200)
      .json({ xLabels, sales, refund, cancle, other, rawCancleOrders });
  } catch (err) {
    res
      .status(500)
      .json({
        message: "Failed to fetch bar chart summary",
        error: err.message,
      });
  }
};
